...
export function activate(context: vscode.ExtensionContext) {
  ...
  server = http.createServer((req, res) => {
    ...
    if (req.method === 'POST' && req.url === '/patch') {
      ...
      req.on('end', async () => {
        try {
          const { path, old: oldText, new: newText } = JSON.parse(body);
  ...
  server.listen(VSCODE_PORT, '127.0.0.1', () => {
    console.log(`Bezot AI extension listening on http://127.0.0.1:${VSCODE_PORT}`);
  });

  // Enregistrer la webview
  const provider = new ChatViewProvider(context.extensionUri);
  context.subscriptions.push(vscode.window.registerWebviewViewProvider('bezot-ai-chat', provider));
